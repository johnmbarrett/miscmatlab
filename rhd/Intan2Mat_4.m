function [amplifier_data, board_dig_in_data, sample_rate] = Intan2Mat_4(filename,f_BPF)
%BP_L = evalin('base','BandPass_Low');
%BP_H = evalin('base','BandPass_High');
%tic;
%filename = [path,file];
...
fprintf(1, '\n');
fprintf('%s \n',['Reading Intan Data File ' filename]);
...
if (data_present)
    fprintf(1, '%0.3f seconds data. Sample rate: %0.2f kS/s.\n', ...
...
if (data_present)
    ...
    fprintf('Read all the blocks %f seconds\n',toc);
    
%     fie = fopen('ABC_171012_175103.rhd','r');
%     fseek(fie,b,-1);
%     
%     tic;
%     A = fread(fie);
%     fprintf('Read the whole file in %f seconds\n',toc);
...
if (data_present)
    ...
    if (num_gaps == 0)
        fprintf(1, 'No missing timestamps.\n');
    else
    ...
    %fprintf(1, 'Notch and High Pass filter...\n');
    notch_filter_frequency = 60;
    Notch_filtered_data = NotchBWfilter(amplifier_data', sample_rate, notch_filter_frequency, 10);
    %notch_filter(amplifier_data', sample_rate, notch_filter_frequency, 10)
    if f_BPF == 1
        BP_L = evalin('base','BandPass_Low');
        BP_H = evalin('base','BandPass_High');
        BP_filtered_data = BandpassBWfilter(Notch_filtered_data, sample_rate, BP_L,BP_H);
        amplifier_data = BP_filtered_data';
        %highpassfilter(amplifier_data(i,:), sample_rate, 300); %%%%%%%%%%%%%%%%%%% 300 Hz Cutoff
    else
        amplifier_data = Notch_filtered_data';
    end
    %     end
end
%fprintf(1, 'RHD to Mat is Done!   %0.1f seconds\n', toc);
%end
...
function out = NotchBWfilter(in, fSample, fNotch, Bandwidth)

bpFilt = designfilt('bandstopiir','FilterOrder',2*2, ...
         'HalfPowerFrequency1',fNotch - Bandwidth/2,'HalfPowerFrequency2',fNotch + Bandwidth/2, ...
         'SampleRate',fSample);
out = filter(bpFilt,in);
out(1,:) = in(1,:);  
out(2,:) = in(2,:);
