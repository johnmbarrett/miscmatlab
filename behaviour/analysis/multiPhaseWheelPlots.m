function multiPhaseWheelPlots(isPlotsToBeSaved)
...
%         uniquePhases = unique(phase);
        ...
        if ~exist('nPhases','var')
            ...
            angleBias = nan(nPhases,nNames);
            turnBias = nan(nPhases,nNames);
            crossoverLatency = inf(nPhases,nNames);
            averageTurnDistance = nan(nPhases,nNames);
            maxTurnDistance = nan(nPhases,nNames);
            totalTurnDistance = nan(nPhases,nNames);
            averageTurnSpeed = nan(nPhases,nNames);
            maxTurnSpeed = nan(nPhases,nNames);
...
%             averageTurnSpeed(sessions(ii),nameIndices(ii)) = totalTurnDistance(sessions(ii),nameIndices(ii))/timestamps(end);
    ...
    for ii = 1:nNames
        ...
        for kk = 1:4
            ...
            lineHandles = gobjects(numel(datas{kk}),1);
            ...
            for jj = 1:numel(datas{kk})
                lineHandles(jj) = plot(vertcat(t{:}),datas{kk}{jj});
            end
