...
[~,sheets] = xlsfinfo(summarySpreadsheet);
...
for ii = 1:numel(sheets)
    %%
    [num,txt,raw] = xlsread(summarySpreadsheet,sheets{ii});
    
    [uniqueCells,~,cellIndices] = unique(num(:,1));
    
    conditions = txt(2:end,2);
    ...
    for jj = 1:nConditions
        for kk = 1:4
            if isempty(txt{jj+1,kk+3})
                continue
            end
            
            % TODO : WaveSurfer
            tokens = regexp(txt{jj+1,kk+3},traceRegex,'tokens');
            traceParts = tokens{1};
            ...
            traces{jj,kk} = arrayfun(@(ll) sprintf('%s%s%04d.xsg',sheets{ii},prefix,ll),(firstTrace:lastTrace)','UniformOutput',false);
        end
    end
    %%
    experimentFolder = [baseFolder sheets{ii}];
...
%             title(sprintf('Expt %s cell %d vsteps %s rep %d',sheets{ii},cellIndices(jj),conditions{jj},intrinsicIndex));            
        ...
        if ~isempty(traces{jj,3})
            ...
            title(sprintf('Expt %s cell %d Isteps %s rep %d',sheets{ii},cellIndices(jj),conditions{jj},istepIndex)); 
...
%             amplitudes = vertcat(stimuli.amplitude);
    ...
    close all
    %%
    missing = find(cellfun(@isempty,Rs))';
    ...
    for jj = 1:nGoodCells
        ...
        title(sprintf('Expt %s cell %d',sheets{ii},cellIndices(jj)));
